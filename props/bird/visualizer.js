...
import { Passaro } from './bird.js';
import { configureTextura } from '../props.js';

const BODY_TEXTURE = "../textures/feather.jpg";
const WING_TEXTURE = "../textures/feather.jpg";
const TAIL_TEXTURE = "../textures/feather.jpg";
const BEAK_TEXTURE = "../textures/trunk.png";
let bodyTexture = null;
let wingTexture = null;
let tailTexture = null;
let beakTexture = null;
// ==================================================================
...
const LUZ = {
  ...
  amb: vec4(1.0, 1.0, 1.0, 1.0), // ambiente
...
function main() {
  ...
  bodyTexture = configureTextura(gl, BODY_TEXTURE);
  wingTexture = configureTextura(gl, WING_TEXTURE);
  tailTexture = configureTextura(gl, TAIL_TEXTURE);
  beakTexture = configureTextura(gl, BEAK_TEXTURE);
...
function crieInterface() {
  document.getElementById("xButton").onclick = function () {
    gPassaro.axis = EIXO_X_IND;
  };
  document.getElementById("yButton").onclick = function () {
    gPassaro.axis = EIXO_Y_IND;
  };
  document.getElementById("zButton").onclick = function () {
    gPassaro.axis = EIXO_Z_IND;
  };
  document.getElementById("pButton").onclick = function () {
    gPassaro.rodando = !gPassaro.rodando;
  };
...
function crieShaders() {
  ...
  gShader.birdBodyVao = setVAO(gPassaro.body);
  gShader.birdWingVao = setVAO(gPassaro.wings[0]);
  gShader.birdTailVao = setVAO(gPassaro.tail);
  gShader.birdBeakVao = setVAO(gPassaro.beak[0]);
...
function render() {
  ...
  eye = vec3(CAMERA_RAIO * Math.sin(gCtx.camTheta[0]) * Math.cos(gCtx.camTheta[1]),
              ...
              CAMERA_RAIO * Math.cos(gCtx.camTheta[0]) * Math.cos(gCtx.camTheta[1]));  gCtx.view = lookAt(eye, at, up);
  gl.uniformMatrix4fv(gShader.uView, false, flatten(gCtx.view));  // renderiza o passaro
  const materials = {
    birdBody: bodyTexture,
    birdWing: wingTexture,
    birdTail: tailTexture,
    birdBeak: beakTexture
  };
  gPassaro.render(gl, gShader, gCtx, mat4(), materials, true);
